...
### 5. Plate.extrudeLine
**<font color="green">`Element.Plate.extrudeLine(elmIDs: list, dir: list, nDiv: int = 1, bDeleteLine: bool = False, stype = 1, mat = 1, sect = 1, angle = 0, group = "", id = None)`</font>**  
Creates meshed Plate elements by extruding existing 1D line elements.

![MIDAS PYTHON](plate_extrudeLine.png)

#### Parameters
* `elmIDs`: List of 1D line element IDs to be extruded (`list(int)`). 
* `dir`: Direction and length of extrusion (eg. `(0,0,2)` - extrude along z direction by 2 units)       
* `nDiv`: No. of divisions along the extrusion.        
* `bDeleteLine`: Whether to delete the original source line elements after extrusion.      
...
Element.Beam.SE((0,0,0),(0,5,0),10,group='#T_Sec')
Element.Beam.SE((0,2.5,0),(0,2.5,-2),4,group='#T_Sec')


Element.Plate.extrudeLine(elmIDs=elemsInGroup('#T_Sec'),
                          dir=(10,0,0),
                          nDiv=20,
                          bDeleteLine=True)
